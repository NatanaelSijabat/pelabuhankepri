...
class LaporanKeuanganController implements IController {
    index = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            const data = await db.LaporanKeuangan.findAll()
    ...
    create = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            ...
            const imagePath = "public/LaporanKeuangan";
            ...
            const data = await db.LaporanKeuangan.create({
                title, image: imageNameWithHash
            })
    ...
    show = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            const { id } = req.params
            const data = await db.LaporanKeuangan.findOne({
                where: { id }
            })
    ...
    update = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            ...
            const data = await db.LaporanKeuangan.findOne({ where: { id } })
            ...
            if (req.file) {
                ...
                const imagePathOld = `public/LaporanKeuangan/${data.image}`
                fs.unlinkSync(imagePathOld)

                const imagePath = `public/LaporanKeuangan/${imageNameWithHash}`;
    ...
    delete = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            ...
            await db.LaporanKeuangan.destroy({ data, where: { id } })
...
export default new LaporanKeuanganController()